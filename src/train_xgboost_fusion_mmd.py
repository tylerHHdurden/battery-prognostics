"""
MMD-aligned counterpart of train_xgboost_fusion.py: identical in every
respect (same 7 BFA-selected HIs + 16-dim fusion embedding, same
battery-level split) except it reads fusion_embeddings_mmd.csv (produced
by train_fusion_encoder_mmd.py) instead of fusion_embeddings.csv.
Additive: writes a parallel `*_fusion_mmd` file set, does not touch the
non-MMD fusion outputs.
"""
...
def main():
    ...
    print(f"[xgb-fusion-mmd] train rows={train_mask.sum()}, test rows={test_mask.sum()}")
    ...
    print(f"[xgb-fusion-mmd] TEST RMSE={rmse:.4f} MAE={mae:.4f} R2={r2:.4f}")
    print(f"[xgb-fusion-mmd] (non-MMD XGBoost-fusion was R2=0.917)")
    ...
    out.to_csv(PRED_DIR / "xgb_fusion_mmd_preds.csv", index=False)

    model.save_model(str(ROOT / "models" / "xgb_soh_fusion_mmd.json"))
    pd.DataFrame([{"model": "XGBoost-fusion-MMD", "rmse": rmse, "mae": mae, "r2": r2,
                   "n_features": len(feature_cols)}]).to_csv(
        PRED_DIR / "xgb_fusion_mmd_metrics.csv", index=False
    )
    print("[xgb-fusion-mmd] DONE")
