...
def render_health_report_tab(ctx: dict, dataset: str, battery_id: str, cycles: list[dict] | None):
    ...
    history_key = f"qa_history::{dataset}::{battery_id}::{ctx.get('cycle_idx', '')}"
    ...
    question = st.chat_input("e.g. \"How confident should I be in this prediction?\"",
                              key=f"qa_input::{history_key}")
    if question:
        history.append(("user", question))
        with st.chat_message("user"):
            st.write(question)
        qa_prompt = build_qa_prompt(qa_context, question)
        with st.chat_message("assistant"):
            with st.spinner("Thinking (Gemini, falling back to Groq if needed)..."):
                answer, qa_provider = call_llm(qa_prompt)
            if answer.startswith("NO_API_KEY") or answer.startswith("API_ERROR"):
                st.info(f"Both Gemini and Groq were unavailable ({answer}) - can't answer "
                        f"free-text questions right now.")
                answer = None
            else:
                st.write(answer)
                st.caption(f"_Answered via {qa_provider}._")
